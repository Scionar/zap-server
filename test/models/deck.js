...
describe('Deck model', () => {
  ...
  describe('#createDeck()', () => {
    it('should create collection index and default collection with cards', (done) => {
    ...
    it('should resolve promise if collection exists in index', (done) => {
      Deck.createDeck(data.cards)
        .then(
          ...
          () => Promise.reject(new Error('Create deck failed.')),
        )
        .then(
          () => done(),
          (msg) => {
            const message = msg || 'collectionExists failed.';
            assert.fail(message);
            done();
          },
        )
